...
class Monster(pygame.sprite.Sprite):
    def __init__(self,name,  speed, x, y, spritesheet, direction):
        super().__init__()
        self.x = x
        self.y = y
    ...
    def animation(self):
        ...
        if self.style == True:
            ...
            if self.index > frame_count:#aby to neslo do nekonecna
                self.index = 0
            self.image = image_cut(self.spritesheet,self.direction_index,int(self.index), 16,16,5)#místo direction jde dát 0 a půjde doleva, příště musím fixnout
...
class Monster2(Monster):
    def __init__(self, name, speed, x, y, spritesheet, direction, scream):
        super().__init__(name, speed, x, y, spritesheet, direction)
        self.scream = scream
